                    ...
                    if (Options.Format)
                    {
                            ...
                            .GetChangedDocuments(onlyGetDocumentsWithTextChanges: true))
                        {
                            ...
                            if (textChanges.Any())
                            {
                                ...
                                //TODO: verify syntax equivalence
                            }
                        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (DocumentId documentId in project.DocumentIds)
            {
                ...
                if (!GeneratedCodeUtility.IsGeneratedCodeFile(document.FilePath))
                {
                    ...
                    if (!WorkspacesUtilities.BeginsWithAutoGeneratedComment(root)
                        && !WorkspacesUtilities.BeginsWithBanner(root, banner))
                    {
                        SyntaxNode newRoot = InsertBanner(root);
            ...
            SyntaxNode InsertBanner(SyntaxNode node)
            {
                switch (node.Language)
                {
                    case LanguageNames.CSharp:
                        return node.PrependToLeadingTrivia(banner.SelectMany(f => Microsoft.CodeAnalysis.CSharp.SyntaxFactory.ParseLeadingTrivia("//" + f + Environment.NewLine)));
                    case LanguageNames.VisualBasic:
                        return node.PrependToLeadingTrivia(banner.SelectMany(f => Microsoft.CodeAnalysis.VisualBasic.SyntaxFactory.ParseLeadingTrivia("'" + f + Environment.NewLine)));
                    default:
                        throw new NotSupportedException($"Language '{node.Language}' is not supported.");
                }
            }
        }
    }
