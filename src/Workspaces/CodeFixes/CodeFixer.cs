        ...
        public async Task FixSolutionAsync(CancellationToken cancellationToken = default)
        {
            ...
            for (int i = 0; i < projects.Length; i++)
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(Options.FileBanner))
                    {
                        project = CurrentSolution.GetProject(project.Id);
                                ...
                                if (!WorkspacesUtilities.BeginsWithAutoGeneratedComment(root))
                                {
                                    switch (root.Language)
                                    {
                                        case LanguageNames.CSharp:
                                            {
                                                if (!WorkspacesUtilities.BeginsWithBanner(
                                                    root,
                                                    Options.FileBannerLines,
                                                    ...
                                                    2))
                                                {
                                                    root = root.PrependToLeadingTrivia(Options.FileBannerLines.SelectMany(f => Microsoft.CodeAnalysis.CSharp.SyntaxFactory.ParseLeadingTrivia("//" + f + Environment.NewLine)));
                                        ...
                                        case LanguageNames.VisualBasic:
                                            {
                                                if (!WorkspacesUtilities.BeginsWithBanner(
                                                    ...
                                                    1))
                                                {
                                                    root = root.PrependToLeadingTrivia(Options.FileBannerLines.SelectMany(f => Microsoft.CodeAnalysis.VisualBasic.SyntaxFactory.ParseLeadingTrivia("'" + f + Environment.NewLine)));
